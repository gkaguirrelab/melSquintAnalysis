%% summarizeEMGByGroup.m
% This routine is intended to produce the summary plots for the
% EMG data collected in the squint study.

paperDir = '~/Desktop';

%% Load up the pupil responses
[ emgResponses, subjectIDsStruct ] = loadEMG;
...
initialPlotPointsToCensor = 0;
endingPlotPointsToCensor = 0;
timebase = 0:0.1:17.5;
...
for stimulus = 1:length(stimuli)
    for contrast = 1:length(contrasts)
        ...
        for group = 1:length(groups)
            ...
            groupMean = nanmean(emgResponses.responseOverTime.(groups{group}).(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]));
        ...
        ylabel('EMG Response (% Change from Baseline)');
        xlim([0 17]);
        xticks([0 5 10 15])
...
export_fig(gcf, fullfile(getpref('melSquintAnalysis', 'melaAnalysisPath'), 'melSquintAnalysis', 'EMG', 'contrastXstimulus_byGroup.pdf'))
...
for stimulus = 1:length(stimuli)
    ...
    for group = 1:length(groups)
        ...
        if group  == 1
            if stimulus == 1
                ylabel({'{\bf\fontsize{15} Light Flux}'; 'Squint'})
            elseif stimulus == 2
                ylabel({'{\bf\fontsize{15} Melanopsin}'; 'Squint'})
                
            elseif stimulus == 3
                ylabel({'{\bf\fontsize{15} LMS}'; 'Squint'})
...
if makeBonusPlots
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            combinedPupilResponses.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [emgResponses.responseOverTime.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]); ...
                emgResponses.responseOverTime.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]); ...
                emgResponses.responseOverTime.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])];
    ...
    for groupNumber = 1:4
        if groupNumber == 1
            pooledResponses = emgResponses.responseOverTime.controls;
            color = 'k';
            groupName = 'Control';
        elseif groupNumber == 2
            pooledResponses = emgResponses.responseOverTime.mwoa;
            color = 'r';
            groupName = 'MwoA';
        elseif groupNumber == 3
            pooledResponses = emgResponses.responseOverTime.mwa;
            color = 'b';
            groupName = 'MwA';
        elseif groupNumber == 4
            for stimulus = 1:length(stimuli)
                for contrast = 1:length(contrasts)
                    pooledResponses.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [emgResponses.responseOverTime.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]); emgResponses.responseOverTime.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])];
                end
            end
    ...
    for ss = 1:length(stimuli)
        for cc = 1:length(contrasts)
            ...
            legend({['Controls, N = ', num2str(size(emgResponses.responseOverTime.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]),1))],['MwoA, N = ', num2str(size(emgResponses.responseOverTime.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]),1))], ['MwA, N = ', num2str(size(emgResponses.responseOverTime.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]),1))]}, 'Location', 'SouthEast')
            legend('boxoff')
            ...
            legend({['Controls, N = ', num2str(size(emgResponses.responseOverTime.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]),1))],['Combined Migraine, N = ', num2str(size(combinedMigrainePupilResponses.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]),1))]},  'Location', 'SouthEast')
            legend('boxoff')
    ...
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            pupilAUC.MwA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.AUC.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            pupilAUC.MwoA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.AUC.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            pupilAUC.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.AUC.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
        end
    end
    ...
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            pupilAUC.CombinedMigraineurs.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [emgResponses.AUC.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]), emgResponses.AUC.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])];
            pupilAUC.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.AUC.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
        end
    end
    ...
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            emgResponses.percentPersistent.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
            emgResponses.percentPersistent.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
            emgResponses.percentPersistent.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
            
            emgResponses.amplitude.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
            emgResponses.amplitude.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
            emgResponses.amplitude.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [];
        end
    end
    ...
    for ss = 1:length(subjectIDs)
        ...
        for stimulus = 1:length(stimuli)
            for contrast = 1:length(contrasts)
                ...
                if strcmp(group, 'c')
                    emgResponses.percentPersistent.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = percentPersistent;
                    emgResponses.amplitude.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = totalResponseAmplitude;
                    
                elseif strcmp(group, 'mwa')
                    emgResponses.percentPersistent.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = percentPersistent;
                    emgResponses.amplitude.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = totalResponseAmplitude;
                    
                    
                elseif strcmp(group, 'mwoa')
                    emgResponses.percentPersistent.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = percentPersistent;
                    emgResponses.amplitude.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])(end+1) = totalResponseAmplitude;
    ...
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            percentPersistent.MwA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.percentPersistent.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            percentPersistent.MwoA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.percentPersistent.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            percentPersistent.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.percentPersistent.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            
            totalResponseAmplitude.MwA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.amplitude.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])*-1;
            totalResponseAmplitude.MwoA.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.amplitude.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])*-1;
            totalResponseAmplitude.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.amplitude.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])*-1;
    ...
    for stimulus = 1:length(stimuli)
        for contrast = 1:length(contrasts)
            percentPersistent.CombinedMigraineurs.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [emgResponses.percentPersistent.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]), emgResponses.percentPersistent.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])];
            percentPersistent.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.percentPersistent.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]);
            
            totalResponseAmplitude.CombinedMigraineurs.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = [emgResponses.amplitude.mwa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]), emgResponses.amplitude.mwoa.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})])].*-1;
            totalResponseAmplitude.Controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]) = emgResponses.amplitude.controls.(stimuli{stimulus}).(['Contrast', num2str(contrasts{contrast})]).*-1;
