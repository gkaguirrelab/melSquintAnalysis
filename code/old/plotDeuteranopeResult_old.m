...
if strcmp(p.Results.whichPlot, 'experimentComparison')
    ...
    for stimulus = 1:length(stimuli)
        ...
        %set(gca, 'OuterPosition', [outerPosition(1), outerPosition(2)+0.3, outerPosition(3)+0.5, outerPosition(4)]);
        %set(gca, 'Position', [position(1), position(2)+0.3, position(3)+0.5, position(4)]);
...
elseif strcmp(p.Results.whichPlot, '400Comparison')
    ...
    for stimulus = 1:length(stimuli)
        ...
        for ss = 1:5
            scatter([1, 2], [experiment1ResultsStruct.(stimuli{stimulus}).Contrast400(ss), experiment2ResultsStruct.(stimuli{stimulus}).Contrast400(ss)], 'Color', 'k')
        end
...
%set(gcf, 'Position', [680 460 968 518]);
