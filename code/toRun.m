trackSubject('MELA_0170', '2019-01-17_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0182', '2019-01-22_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0179', '2019-01-15_session_1', 'protocol', 'Screening', 'resume', true)
trackSubject('MELA_0181', '2019-01-16_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0177', '2019-01-08_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0168', '2018-12-11_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0178', '2019-01-10_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0174', '2018-12-17_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0175', '2018-12-14_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0173', '2018-12-13_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0168', '2018-12-11_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0171', '2018-12-05_session_1', 'Protocol', 'Screening', 'resume', true);
trackSubject('MELA_0166', '2018-11-30_session_1', 'Protocol', 'Screening', 'resume', true);
trackSubject('MELA_0167', '2018-11-30_session_1', 'Protocol', 'Screening', 'resume', true);
trackSubject('MELA_0169', '2018-12-03_session_1', 'Protocol', 'Screening', 'resume', true);
trackSubject('MELA_0174', '2018-12-17_session_1', 'Protocol', 'Screening', 'resume', true)
trackSubject('MELA_0169', '2019-01-24_session_2', 'resume', true)
trackSubject('MELA_0169', '2019-01-24_session_1', 'resume', true)
trackSubject('MELA_0181', '2019-01-23_session_1', 'resume', true)
trackSubject('MELA_0181', '2019-01-23_session_2', 'resume', true)
trackSubject('MELA_0179', '2019-01-18_session_2', 'resume', true)
trackSubject('MELA_0179', '2019-01-18_session_1', 'resume', true)
trackSubject('MELA_0128', '2019-01-15_session_4', 'resume', true)
trackSubject('MELA_0128', '2019-01-15_session_3', 'resume', true)
trackSubject('MELA_0166', '2019-01-11_session_4', 'resume', true)
trackSubject('MELA_0174', '2019-01-16_session_4', 'resume', true)
trackSubject('MELA_0174', '2019-01-16_session_3', 'resume', true)
trackSubject('MELA_0166', '2019-01-11_session_3', 'resume', true)
trackSubject('MELA_0174', '2019-01-09_session_2', 'resume', true)
trackSubject('MELA_0174', '2019-01-09_session_1', 'resume', true)
trackSubject('MELA_0160', '2018-11-09_session_4', 'resume', true);
trackSubject('MELA_0158', '2018-11-13_session_1', 'resume', true);
trackSubject('MELA_0150', '2018-11-12_session_4', 'resume', true); 
%trackSubject('MELA_0150', '2018-11-12_session_3', 'resume', true);
trackSubject('MELA_0153', '2018-11-19_session_4', 'resume', true);
trackSubject('MELA_0153', '2018-11-19_session_3', 'resume', true);
trackSubject('MELA_0162', '2018-11-15_session_2', 'resume', true);
trackSubject('MELA_0162', '2018-11-15_session_1', 'resume', true);
trackSubject('MELA_0158', '2018-11-13_session_2', 'resume', true);
trackSubject('MELA_0155', '2018-11-28_session_4', 'resume', true);
trackSubject('MELA_0167', '2018-12-04_session_1', 'resume', true);
trackSubject('MELA_0167', '2018-12-04_session_2', 'resume', true);
trackSubject('MELA_0167', '2018-12-11_session_4', 'resume', true);
trackSubject('MELA_0167', '2018-12-11_session_3', 'resume', true)


trackSubject('MELA_0169', '2019-02-08_session_3', 'resume', true);
trackSubject('MELA_0169', '2019-02-08_session_4', 'resume', true);
trackSubject('MELA_0181', '2019-02-06_session_3', 'resume', true);
trackSubject('MELA_0181', '2019-02-06_session_4', 'resume', true);
trackSubject('MELA_0164', '2019-02-04_session_2', 'resume', true);
trackSubject('MELA_0164', '2019-02-04_session_1', 'resume', true);




