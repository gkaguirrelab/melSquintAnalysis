...
for ss = 1:length(averageResponseStructAccumulator)
    for stimulus = 1:length(stimuli)
        ...
        response100 = averageResponseStructAccumulator{ss}.(stimuli{stimulus}).Contrast100(firstIndexToPlot:lastIndexToPlot) - yoffset*(ss-1);
        response200 = averageResponseStructAccumulator{ss}.(stimuli{stimulus}).Contrast200(firstIndexToPlot:lastIndexToPlot) - yoffset*(ss-1);
        response400 = averageResponseStructAccumulator{ss}.(stimuli{stimulus}).Contrast400(firstIndexToPlot:lastIndexToPlot) - yoffset*(ss-1);
